    ...
    if [ "$TELEGRAM_ENABLED" = true ]; then
        ...
        if prompt_telegram_post; then
            ...
            if select_telegram_groups; then
                ...
                SUMMARY_MESSAGE="🚀 *New AmbatuBuild Available!*%0A%0A"
                SUMMARY_MESSAGE+="📦 *Project:* $PROJECT_NAME%0A"
                ...
                for i in "${!SELECTED_GROUPS[@]}"; do
                    ...
                    if [ -f "$BUILD_DIR/$ZIP_NAME" ]; then
                        caption="📱 $PROJECT_NAME - $VERSION ($BUILD_TYPE)"

                        if send_to_telegram "$BUILD_DIR/$ZIP_NAME" "$caption" "$chat_id"; then
                            ((upload_success++))

                            # Send completion message
                            COMPLETION_MESSAGE="✅ *Upload Complete!*%0A%0AProject build uploaded successfully to $group_name."
                            send_message_to_telegram "$COMPLETION_MESSAGE" "$chat_id"
                        else
                            # Send failure message
                            FAILURE_MESSAGE="❌ *Upload Failed*%0A%0AThere was an issue uploading the project build to $group_name."
